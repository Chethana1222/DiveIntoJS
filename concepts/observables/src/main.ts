...
//         Observable.from([42,78,1,22]).delay(10).subscribe(val=>console.log(`Delay: ${val}`));
...
// //End of Inbuilt methods of observables


/* 
Scenario:

Create an observable object to generate an random number between 0 to 10 for every one second
number >=2 keep on produce random number and send it
...
var obCreateObj: Observable<number> = new Observable(observer=>{
    ...
    setInterval(()=>{
        ...
        if(randomNumber>=2){
            ...
            throw `NumberZeroException Occured : ${randomNumber}`;
        }
        else
        observer.complete();
        }
